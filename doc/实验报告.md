...
- 度量值 -> 定位字符。通过这些度量值计算出。
  ...
  https://learnopengl-cn.github.io/img/06/02/glyph.png pic addr（图自己p一下）
...
- 根据每个点到火焰中心的距离（火焰中心位置在函数调用的时候传入）计算出每一个点要增加的 density。使用 smoothstep 计算出 factor u，u 在 0、1 之间，与调用函数时传入的 value 相乘得到要增加的 density 的值。距离远的计算得到的值较小，距离近的值较大。
  ...
  ![1563513864452](C:\Users\yali\AppData\Roaming\Typora\typora-user-images\1563513864452.png)
...
- 涡度约束
  ...
  ![1563513987181](C:\Users\yali\AppData\Roaming\Typora\typora-user-images\1563513987181.png)
